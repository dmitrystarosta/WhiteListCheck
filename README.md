...
**Виджет на рабочий стол.** Значок-индикатор в цвете последней проверки. Тап по виджету запускает новую проверку прямо с рабочего стола, не открывая приложение; под значком показано время последней проверки. Если данные старше часа, виджет тускнеет — пора перепроверить. Размер настраивается, работает и на телефоне, и на планшете.

<p align="center">
  <img src="/docs/badges/whitelistcheck_widget.jpg" alt="Виджет «Белый список?» на рабочем столе" title="Виджет «Белый список?» на рабочем столе" width="240">
</p>

Цвет значка — это вердикт последней проверки:

| Цвет | Что означает |
|---|---|
| Зелёный | всё в норме, интернет работает как обычно |
| Красный | включён режим белого списка |
| Синий | открывается даже заблокированное — VPN или вы вне РФ |
| Серый | интернета нет |

До первой проверки значок нейтральный (коричневый) с подписью «нажмите» — коснитесь его, чтобы проверить сеть.
...
- Локальный журнал проверок с выгрузкой — чтобы фиксировать, когда в регионе включали и выключали белый список
