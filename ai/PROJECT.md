...
Каталог F-Droid — отдельный GitLab-репозиторий gitlab.com/fdroid/fdroiddata;
приложение добавляется через merge request (!46918), файл метаданных
metadata/ru.netstatus.app.yml. F-Droid собирает приложение из исходников САМ,
из строго указанного коммита (commit:), и при совпадении с публичным релизом
раздаёт APK с подписью автора (reproducible build). Для этого в репозиторий
закоммичен Gradle wrapper (иначе сборка падает: «No suitable gradle version»).
История, правила и текущий yml — в ai/FDROID.md.
...
- **Секреты GitHub + assembleRelease в CI** — после инцидента
  с утечкой паролей.
- **Google Play — идём (с 08.09.2026)**: аккаунт создан, приложение
  заведено, приоритет по-прежнему RuStore. Ключевое решение — **отдать
  Google свой ключ подписи** (Play App Signing через PEPK), а не
  разрешать генерацию нового: иначе Play-версия стала бы отдельным
  приложением с другой подписью, и пользователь не смог бы переставить
  его между RuStore и Play без удаления. Второе решение — отключить
  automatic protection (проверку источника установки): она подталкивает
  пользователей из других магазинов в Play, что противоречит
  распространению через RuStore/F-Droid/GitHub. Ради Play поднят
  targetSdk до 36 (требование Play с 31.08.2026) — оно же затронуло
  все каналы, но им не мешает. Детали — ai/GOOGLEPLAY.md.
