...
if (videoFacade) {
  videoFacade.addEventListener('click', () => {
    ...
    // muted=0 — просим плеер не приглушать звук. Браузер вправе это
    // проигнорировать, но клик по заглушке даёт пользовательскую
    // активацию, поэтому чаще всего звук остаётся включённым.
    const params = 'autoplay=1&muted=0';
    iframe.src = src.includes('?') ? `${src}&${params}` : `${src}?${params}`;
